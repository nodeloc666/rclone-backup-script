...
# Version: 8.15 (Auto-install Dependencies)
#
# Description:
#   Adds functionality to automatically check, detect missing, and install
#   required system dependencies (zip, rclone, curl, jq) using the appropriate
#   package manager (apt, yum, dnf) or rclone's official install script.
#   Ensures the script is run as root for installation.
# ==============================================================================
...
readonly REQUIRED_DEPS=("zip" "rclone" "curl" "jq") # Rclone is now part of auto-install
...
handle_critical_failure() {
  ...
  SUCCESS_FLAG=1
  # For critical failures where we cannot proceed at all (e.g., not root, cannot install deps).
  # We exit immediately. The trap ensures script_final_exit still runs for logging.
  exit 1
}
...
# --- New Function: Detect Package Manager ---
...
# --- Modified Function: check_dependencies ---
check_dependencies() {
  log_message "Checking for required dependencies: ${REQUIRED_DEPS[*]}..."

  # Check for root privileges first, as installation requires it.
  if [[ $EUID -ne 0 ]]; then
    handle_critical_failure "This script must be run as root to install missing dependencies. Current UID: $EUID"
  fi

  local pkg_manager=$(detect_package_manager)
  if [[ "${pkg_manager}" == "unknown" ]]; then
      handle_critical_failure "Could not detect a supported package manager (apt, yum, dnf). Automatic dependency installation is not possible."
  fi
  ...
  log_message "The following dependencies are missing and will be installed: ${missing_deps[*]}"

  # Attempt to install missing dependencies
  for dep_to_install in "${missing_deps[@]}"; do
    log_message "Attempting to install '${dep_to_install}'..."
    local install_cmd_output=""
    local pkg_name="${dep_to_install}" # Default package name is the same as the command

    case "${dep_to_install}" in
      "rclone")
        # Rclone has a dedicated install script for robustness
        log_message "Installing rclone via official script..."
        install_cmd_output=$(curl -s "https://rclone.org/install.sh" | bash 2>&1)
        ;;
      "zip")
        # Zip is usually just 'zip'
        ;;
      "jq")
        # JQ is usually just 'jq'
        ;;
      # Add other specific package name mappings if needed, e.g., "tar" could be "tar"
      # "curl" is usually "curl"
    esac

    if [ "${dep_to_install}" != "rclone" ]; then # For packages other than rclone, use standard pkg manager
        case "${pkg_manager}" in
            "apt")
                log_message "Running apt update before installing ${pkg_name}..."
                if ! sudo apt-get update -y > /dev/null 2>&1; then # Update package lists silently
                    log_message "WARNING: apt update failed. This may cause issues with installing ${pkg_name}."
                fi
                install_cmd_output=$(sudo apt-get install -y "${pkg_name}" 2>&1)
                ;;
            "yum")
                install_cmd_output=$(sudo yum install -y "${pkg_name}" 2>&1)
                ;;
            "dnf")
                install_cmd_output=$(sudo dnf install -y "${pkg_name}" 2>&1)
                ;;
        esac
    fi

    if command -v "${dep_to_install}" &>/dev/null; then
      log_message "Successfully installed '${dep_to_install}'."
    else
      log_message "ERROR: Failed to install '${dep_to_install}'. Output: ${install_cmd_output}"
      install_success=false
      # Accumulate global failure reason if multiple dependencies fail
      if [ -z "${GLOBAL_FAILURE_REASON}" ]; then
          GLOBAL_FAILURE_REASON="Failed to install required dependency: ${dep_to_install}. Check logs for details."
      else
          GLOBAL_FAILURE_REASON="${GLOBAL_FAILURE_REASON}; Failed to install: ${dep_to_install}."
      fi
      SUCCESS_FLAG=1 # Mark overall script as failed
    fi
  done

  if ! "${install_success}"; then
      handle_critical_failure "One or more required dependencies could not be automatically installed. Please install them manually and try again."
  fi
  
  log_message "All necessary dependencies are now available."
  return 0
}
...
# ==============================================================================
main() {
  # Trap 'EXIT' ensures script_final_exit is run even if there's an error and script exits prematurely.
  trap script_final_exit EXIT
  # Redirect all stdout and stderr to the log file from this point forward.
  # This must happen before any output that needs to be logged.
  exec >> "${LOG_FILE}" 2>&1
  ...
  # Dependencies are checked and installed here
  check_dependencies
  ...
  # It's recommended to set ENCRYPTION_PASSWORD in the environment before running the script
  ...
  # Initialize status messages for skipped operations based on the backup mode *before* operations begin.
  initialize_status_messages_for_mode
  ...
  if zip -r -e -P "${ENCRYPTION_PASSWORD}" -q "${CURRENT_ZIP_FILE}" "${SOURCE_DIR}"; then
    ...
    # Exit here to prevent proceeding with a failed pack, allows script_final_exit to run thanks to trap.
    exit 1
  fi
  unset ENCRYPTION_PASSWORD # 清除敏感信息
  ...
  if [[ ("${BACKUP_MODE}" == "remote_only" || "${BACKUP_MODE}" == "local_and_remote") && "${PACK_STATUS_MESSAGE}" == "Success" && -n "${RCLONE_TARGET}" ]]; then
    ...
    if rclone copy "${CURRENT_ZIP_FILE}" "${RCLONE_TARGET}" --checksum --transfers=4 --buffer-size 16M; then
      ...
      # Only set GLOBAL_FAILURE_REASON if it hasn't been set by a prior critical failure
    ...
    # UPLOAD_STATUS_MESSAGE already set to "Skipped (Local only mode)" or similar by initialize_status_messages_for_mode
    # or will remain empty for modes where it might happen but didn't (e.g., failed pack).
    # If the pack failed for remote_only or local_and_remote, this log is important.
    ...
    if [[ "${BACKUP_MODE}" == "local_only" || "${BACKUP_MODE}" == "local_and_remote" ]]; then
      ...
      if [ "${PACK_STATUS_MESSAGE}" == "Success" ]; then
        if ! cleanup_old_backups_on_local; then
          SUCCESS_FLAG=1 # Mark failure but continue, as this is cleanup
        fi
      else
      ...
      if rm -f "${CURRENT_ZIP_FILE}"; then
        LOCAL_CLEANUP_STATUS_MESSAGE="Cleaned temporary file" # This overrides any default "skipped" message
      else
        ...
        SUCCESS_FLAG=1 # Mark failure but continue
      fi
    fi
  else
  ...
  if [[ ("${BACKUP_MODE}" == "remote_only" || "${BACKUP_MODE}" == "local_and_remote") && -n "${RCLONE_TARGET}" ]]; then
    if ! cleanup_old_backups_on_remote; then
      ...
      # REMOTE_CLEANUP_STATUS_MESSAGE for local_only mode is initialized by initialize_status_messages_for_mode.
      # For other modes if not entered, it will remain what it was initialized to or empty.
